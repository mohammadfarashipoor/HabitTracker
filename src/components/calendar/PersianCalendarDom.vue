<template>
  <div class="persian-calendar">
    ...
    <table class="persian-calendar__table">
      <th>
        <tr>
          <td class="persian-calendar__weekday-names">ش</td>
          <td class="persian-calendar__weekday-names">ی</td>
          <td class="persian-calendar__weekday-names">د</td>
          <td class="persian-calendar__weekday-names">س</td>
          <td class="persian-calendar__weekday-names">چ</td>
          <td class="persian-calendar__weekday-names">پ</td>
          <td class="persian-calendar__weekday-names">ج</td>
        </tr>
      </th>
...
onMounted(() => {
  class PersianCalendar {
    currnetMonth = ref();
    currnetYear = ref();
    checkCurrentMonth = ref(false);
    // first day and last day of month
    firstDay = ref();
    lastDay = ref();
    constructor() {
      this.currnetMonth = ref(null);
      this.currnetYear = ref(null);
    }
    static months = [
      "فروردین",
      "اردیبهشت",
    ...
    setCurrentdates() {
      this.currnetMonth.value = this.getPersianToday().monthNumber;
      this.currnetYear.value = this.getPersianToday().year;
    }
    ...
    createDaysOfMonth(monthNumber) {
      ...
      if (monthNumber <= 6) {
        ...
        createDays(30, days);
        return days;
      } else if (monthNumber === 12 && isLeapYear(this.currnetYear.value)) {
        createDays(30, days);
        return days;
      } else if (monthNumber === 12 && !isLeapYear(this.currnetYear.value)) {
        createDays(29, days);
    ...
    setNextMonth() {
      if (this.currnetMonth.value == 12) {
        this.currnetMonth.value = 0;
        this.currnetYear.value++;
      }
      persianCalendar.createDaysOfMonth(++this.currnetMonth.value);
      if (this.lastDay.value >= 6) {
        this.lastDay.value = -1;
      }
      const dateObject = {
        day: 1,
        dayOfWeek: PersianCalendar.dayNames[this.lastDay.value + 1],
        monthNumber: this.currnetMonth.value,
      };
      this.firstDay.value = this.lastDay.value + 1;
      this.lastDay.value = getLastDayOfMonth(dateObject, this.currnetYear.value)[0];
      this.createCalendar();
    }

    setPreviousMonth() {
      if (this.currnetMonth.value == 1) {
        this.currnetMonth.value = 13;
        this.currnetYear.value--;
      }
      persianCalendar.createDaysOfMonth(--this.currnetMonth.value);
      if (this.firstDay.value <= 0) {
        this.firstDay.value = 7;
      }
      let dateObject;
      if (this.currnetMonth.value < 1) {
        if (isLeapYear(this.currnetYear.value - 1)) {
          dateObject = {
            day: 30,
            dayOfWeek: PersianCalendar.dayNames[this.firstDay.value - 1],
            monthNumber: this.currnetMonth.value,
          };
        } else {
          dateObject = {
        ...
        dateObject = {
          day: PersianCalendar.sizeOfMonths[this.currnetMonth.value - 1],
          dayOfWeek: PersianCalendar.dayNames[this.firstDay.value - 1],
          monthNumber: this.currnetMonth.value,
        };
      }
      this.lastDay.value = this.firstDay.value - 1;
      this.firstDay.value = getFirstDayOfMonth(dateObject)[0];
      this.createCalendar();
    }
    // craete calendar
    createCalendar() {
      ...
      calendar.innerHTML = "";
      const daysOfMonth = this.createDaysOfMonth(this.currnetMonth.value);
      // set first day and last day in current month
      if (!this.checkCurrentMonth.value) {
        this.firstDay.value = getFirstDayOfMonth(this.getPersianToday())[0] + 1;
        this.lastDay.value =
          getLastDayOfMonth(this.getPersianToday(), this.currnetYear.value)[0] + 1;
        this.checkCurrentMonth.value = true;
      }
      const daysInWeek = 7;
      let currentDay = 1;
      let totalRow = 5;
      if (this.firstDay.value >= 5 && (this.lastDay.value == 0 || this.lastDay.value == 1)) {
        totalRow = 6;
      }
      // set next and prev month days
      const prevMonth = this.currnetMonth.value === 1 ? 12 : this.currnetMonth.value - 1;
      const prevMonthDays = this.createDaysOfMonth(prevMonth);
      const nextMonth = this.currnetMonth.value === 12 ? 1 : this.currnetMonth.value + 1;
      ...
      for (let week = 0; week < totalRow; week++) {
        ...
        for (
          ...
          if (week === 0 && dayOfWeekIndex < this.firstDay.value) {
            const prevMonthDay =
              prevMonthDays[
                prevMonthDays.length - (this.firstDay.value - dayOfWeekIndex)
              ];
          ...
          // set cells in current month
            ...
            const cellIdentifier = `${this.currnetYear.value}-${this.currnetMonth.value}-${cell.textContent}`;
  ...
  document.addEventListener("click", (event) => {
    ...
    if (clickedCell.tagName === "TD" && clickedCell.textContent !== "") {
      ...
      const clickedMonth = persianCalendar.currnetMonth.value;
      const clickedYear = persianCalendar.currnetYear.value;
  ...
  function updateCurrentDate() {
    const currentMonth = persianCalendar.currnetMonth.value;
    const currentYear = persianCalendar.currnetYear.value;
    currentDateElement.textContent = `${
      PersianCalendar.months[currentMonth - 1]
    } ${persianToEnglishNumber(currentYear)}`;

    persianCalendar.addEvent("1402-6-9");
    persianCalendar.addEvent("1402-6-13");
    persianCalendar.addEvent("1402-7-25");
    persianCalendar.addEvent("1402-7-15");
